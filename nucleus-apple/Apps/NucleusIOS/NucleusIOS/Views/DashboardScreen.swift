...
struct DashboardWorkspaceScreen: View {
    ...
    private func greetingLine(asOf date: Date) -> String {
        let firstName = DashboardGreeting.firstName(from: viewModel.iCloudSync.accountName)
        return "\(DashboardGreeting.timeOfDay(now: date)), \(firstName)!"
    }
    ...
    private var publicHolidaySection: some View {
        DashboardPublicHolidayCard(
            holiday: holidayService.nextHoliday,
            locationLabel: holidayService.locationLabel,
